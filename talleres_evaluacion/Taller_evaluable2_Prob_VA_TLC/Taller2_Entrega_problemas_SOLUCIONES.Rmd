---
title: "Taller 2 entrega problemas  - Probabilidad, Variables aleatoria , distribuciones notables y Teorema Central del Límite"
...
Entonces $E\left(\sum_{i=1}^{48} X_i\right)=48\cdot \frac{a}{2}=24\cdot a$ , $\sigma^2_{\sum_{i=1}^{48}X_i}=48^2\cdot \frac{a^2}{12}=4\cdot a^2$ y por lo tanto la deviación típica de la suma es $\sigma_{\sum_{i=1}^{48}X_i}=+\sqrt{\sigma^2_{\sum_{i=1}^{48} X_i}}=\sqrt{4\cdot a^2}=2\cdot a.$
...
Luego  $P\left(\sum_{i=1}^{48} X_i >a\right)=1-P\left(\sum_{i=1}^{48}\leq a \right)\approx 1-P\left(Z\leq \frac{a-24 \cdot a}{2\dot a}\right)=1-P\left(Z\leq \frac{-23}{2}\right)\approx 1-1=0.$
