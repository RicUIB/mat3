...
**Apartado 1.** El cógido R carga en las variables G1 y G2 la variables `x` de dos data frames de un servidor bioinfo.uib.es.

Luego cácula los estadísticos básicos para  realizar las siguientes preguntas. Pra los tamaños muestrales $n_1$ y $n_2$  se omiten los valores `NA` antes de asignar la `length` de los arrays. También se calulan las medias y las desviaciones típicas muestrales omitiendo (si es que hay)  los valores no disponibles.
...
```{r}
barplot(rbind(frec_esp_benford,frec_obs_primer),beside=TRUE,col=c("red","blue"))
# añadir legend
```
...
```{r}
barplot(rbind(frec_esp_uniforme,frec_obs_segundo),beside=TRUE,col=c("red","blue"))
# añadir legend
```
...
# pasar otros test si se rechaza
ad.test(datos_interes$interes[datos_interes$ciudad==6])
```
...
```{r}
bartlett.test(datos_interes$interes,as.factor(datos_interes$ciudad))
```
