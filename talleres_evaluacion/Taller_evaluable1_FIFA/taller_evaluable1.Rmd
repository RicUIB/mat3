...
names(sort(table(datos_o$club[1:10]),decreasing = TRUE))[1:6]
```
...
```{r}
datos6=datos[datos$club %in% names(sort(table(datos_o$club[1:10]),decreasing = TRUE))[1:6],]
unique(datos6$club)
```

## Pregunta 3

Calcular la media y la desviación típica   del sueldo de cada equipo del dataframe `fifa20_best_shooting`. 
