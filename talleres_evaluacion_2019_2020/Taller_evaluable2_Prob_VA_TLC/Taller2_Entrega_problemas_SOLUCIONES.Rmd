...
P\left(X\leq \frac{1}{2}\right)- P\left(X < -\frac{1}{2}\right)=
...
Hagamos el dibujo con R

```{r}
fY=function(y) { sapply(y, FUN=
                          function(x) {
                            if(x>0 & x<1){2*(1-x)} else {0}
                            }
                        )}

### cuidado curve pide que la función tenga la variable de nombre x
curve(fY(x),xlim=c(-0.25,1.25),xlab="y",
      ylab=expression("f_Y(y)"), 
      main="Gráfico de la  función de\n  densidad de la variable Y",
      col="blue")
```
...
\displaystyle
\begin{array}{rl}
P(X \mbox{ impar})&=\displaystyle\sum_{k=0}^{\infty}P_X(X=2\cdot k+1)=
\displaystyle\sum_{k=0}^{\infty} (1-p)^{(2\cdot k+1)-1}\cdot p\\
&= p\cdot \displaystyle\sum_{k=0}^{\infty} \left((1-p)^2\right)^{k}=
...
Con los datos del problema sabemos que $P(\mbox{Pelirrojo})=\frac{1}{1000}$. Suponiendo independencia entre el color del pelo de los  5000 nacimientos la variable 
...
Entonces $E\left(\sum_{i=1}^{48} X_i\right)=48\cdot \frac{a}{2}=24\cdot a$ , $\sigma^2_{\sum_{i=1}^{48}X_i}=48\cdot \frac{a^2}{12}=4\cdot a^2$ y por lo tanto la desviación típica de la suma es $\sigma_{\sum_{i=1}^{48}X_i}=+\sqrt{\sigma^2_{\sum_{i=1}^{48} X_i}}=\sqrt{4\cdot a^2}=2\cdot a.$

Límite sabemos que 
Bajo estas condiciones y por el Teorema Central del Límite sabemos  que 
